...
async function autoCommand(action) {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: (requestId, action) => new Promise((resolve, reject) => {
      const response = "__primovySkipAdResponseV1";
      const timeout = setTimeout(() => {
        window.removeEventListener(response, onResponse);
        reject(new Error("Firefox bridge timed out"));
      }, 1500);
      ...
      window.dispatchEvent(new CustomEvent("__primovySkipAdRequestV1", {
        detail: JSON.stringify({ requestId, action })
      }));
    }),
